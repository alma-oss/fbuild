...
let runtimeIdentifierTests =
    testList "RuntimeIdentifier" [
        ...
        test "should use the runtime-provided RID when an identifier is detected" {
            let result = RuntimeIdentifier.detect () |> RuntimeIdentifier.value

            Expect.equal result RuntimeInformation.RuntimeIdentifier "Detected runtime identifier should match .NET"
        }
    ]
    ...
    testList "RuntimeConfiguration" [
        test "should resolve the detected runtime when it is a release target" {
            let configuration: RuntimeConfiguration = {
                RuntimeTargets = [ Custom (RuntimeIdentifier.value (RuntimeIdentifier.detect ())) ]
                RuntimeMode = RuntimeMode.AutoDetect
            }

            let result = RuntimeConfiguration.resolve configuration

            Expect.equal result (Ok (Some (RuntimeIdentifier.detect ()))) "Detected RID listed as a release target should be accepted"
        }
        ...
        test "should return UnsupportedRuntime error when the detected runtime is not a release target" {
            ...
            Expect.equal
                result
                (Error (RuntimeConfigurationError.UnsupportedRuntime (RuntimeIdentifier.detect ())))
